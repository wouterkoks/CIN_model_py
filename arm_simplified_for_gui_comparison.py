...
arm.gammatheta = 3.4e-3      # free atmosphere potential temperature lapse rate [K m-1]
...
arm.q          = 15.8e-3   # initial mixed-layer specific humidity [kg kg-1]
arm.dq         = -0.2e-3   # initial specific humidity jump at h [kg kg-1]
arm.gammaq     = -2e-6      # free atmosphere specific humidity lapse rate [kg kg-1 m-1]
arm.advq       = 0.        # advection of moisture [kg kg-1 s-1]
arm.wq         = None      # surface kinematic moisture flux [kg kg-1 m s-1]
arm.q_ft0      = arm.q + arm.dq + 2e-6 * 140  # free tropospheric profile extrapolated to surface[kg kg-1]
...
arm.tstart     = 0      # time of the day [h UTC]
...
# print(wtheta)
time   = np.linspace(0.001, 12.001, 50) 


wtheta = 0.125 * np.sin(np.pi * time / 12)
wq     = 0.18e-3 * np.sin(np.pi * time / 12)
time  *= 3600.
...
plt.plot(r3.out.t, r3.out.h, label='ARM Cu+CIN+Sft')
if PLOT_LES:
...
plt.figure()
# plt.plot(r1.out.t, r1.out.q*1000., label='ARM Simple', color='tab:blue')
plt.plot(r2.out.t, r2.out.q*1000., label='ARM Cu', color='green')
plt.plot(r3.out.t, r3.out.q*1000., label='ARM Cu+CIN+Sft', color='blue')
plt.xlabel('time [h]', fontsize=fsize)
...
plt.figure()
plt.plot(r1.out.t, r1.out.ac*r1.out.w_lfc)
plt.plot(r1.out.t, r1.out.M)
plt.show()
